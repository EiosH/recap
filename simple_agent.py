...
from outline_utils import list_outlines, outline_label, outline_path
...
CHAT_SYSTEM_TEMPLATE = """你是大学课程助教。下面是一份基于课堂录音整理的课程知识提纲，请仅依据提纲内容回答学生问题。

要求：
1. 优先使用提纲中的知识点作答；提纲未涉及的内容请明确说明「提纲中未提及」；
2. 回答简洁准确，必要时用条目列出；
3. 使用中文。
...
def format_messages_prompt(tokenizer, messages: list[dict]) -> str:
    """多轮对话 chat template。"""
    return tokenizer.apply_chat_template(
        messages,
        tokenize=False,
...
def invoke_llm(
    ...
    return _pipe_generate(
        llm, prompt_text, max_new_tokens=max_new_tokens
    )
...
def print_menu() -> None:
    ...
    for k, cfg in MODEL_REGISTRY.items():
        ...
        print(f"        {cfg['label']}")
    print("\n命令:")
    print("  run          生成提纲（保存为 *-outline-{模型}.txt）")
    print("  chat         基于最近提纲，选择模型后命令行对话")
    print("  30b/20b/10b  切换模型")
    print("  gpu          刷新显卡信息")
    print("  quit         退出")


def _pick_outline() -> Path | None:
    outlines = list_outlines(DATA_DIR, CHUNK_RUN_DIR)
    if not outlines:
        print("未找到任何提纲。请先 run 生成。")
        return None

    print("\n可用提纲（按时间倒序）:")
    for i, p in enumerate(outlines[:10], 1):
        mtime = datetime.fromtimestamp(p.stat().st_mtime).strftime("%Y-%m-%d %H:%M")
        print(f"  [{i}] {outline_label(p)}  ({mtime})  {p}")

    choice = input("选择提纲编号 [1=最新，回车=最新] > ").strip()
...
def _pick_chat_model(default_key: str) -> str:
    print("\n选择对话模型:")
    for k, cfg in MODEL_REGISTRY.items():
        mark = " ← 当前" if k == default_key else ""
        print(f"  [{k}]  {cfg['model_id']}{mark}")
    raw = input(f"模型档位 [回车={default_key}] > ").strip().lower()
    if not raw:
        return default_key
    if raw in MODEL_REGISTRY:
        return raw
    print(f"无效选择，使用 {default_key}。")
    return default_key
...
def run_chat_session(
    ...
    outline_for_ctx = _truncate_to_tokens(
        tokenizer, outline_text, MAX_OUTLINE_TOKENS_CHAT
    )
    if len(outline_for_ctx) < len(outline_text):
        print(f"  提示: 提纲较长，对话上下文已截断至 {MAX_OUTLINE_TOKENS_CHAT} tokens")

    system_content = CHAT_SYSTEM_TEMPLATE.format(outline=outline_for_ctx)
    history: list[dict] = []

    print("\n" + "=" * 60)
    print("课程助教对话")
    print(f"  提纲: {outline_label(outline_file)}")
    print(f"  模型: {model_key} ({MODEL_REGISTRY[model_key]['model_id']})")
    print("  输入 exit / back 返回主菜单，clear 清空对话历史")
    print("=" * 60 + "\n")
    ...
    while True:
        ...
        t0 = time.perf_counter()
        try:
            reply = invoke_chat(llm, tokenizer, messages)
        except torch.OutOfMemoryError:
            _cuda_cleanup()
            print("[OOM] 显存不足。请尝试 10b 模型，或输入 clear 清空历史后重试。\n")
            continue
...
def main() -> None:
    ...
    while True:
        ...
        if raw in MODEL_REGISTRY:
            ...
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            print(f"已切换到 {current_key}。")
            continue
        if raw == "chat":
            outline_file = _pick_outline()
            if outline_file is None:
                continue
            chat_model = _pick_chat_model(current_key)
            if loaded_key != chat_model or llm is None:
                if llm is not None and torch.cuda.is_available():
                    del llm
                    torch.cuda.empty_cache()
                llm, load_time_sec, quant_mode = build_llm(chat_model)
                loaded_key = chat_model
                current_key = chat_model
            assert llm is not None
            tokenizer = llm.pipeline.tokenizer
            outline_text = outline_file.read_text(encoding="utf-8").strip()
            run_chat_session(
                llm, tokenizer, outline_text, outline_file, chat_model
            )
            continue
        if raw != "run":
            print("未知命令。可用: run | chat | 30b/20b/10b | gpu | quit")
            continue
        ...
        out_name = outline_path(DATA_DIR, lecture_path.stem, current_key)
        out_name.write_text(metrics.answer, encoding="utf-8")
        print(f"提纲已保存: {out_name}")
